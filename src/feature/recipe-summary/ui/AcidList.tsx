    ...
    return (
        <div className={styles.wrapper}>
            ...
            <ul className={styles.list}>
                {selectedAcids.map((acid, index) => (
                    <li key={acid.id} className={getRowClass(index)}>
                        ...
                        <span className={styles.gram}>{formatNumber(acid.mass, 2)} {measureInputTypeMeta[measureInput].ru.short}</span>
                    </li>
                ))}
            </ul>
            ...
            <div className={styles.summary}>
                ...
                <span className={styles.summaryGram}>{formatNumber(totalAcidAmount, 2)} {measureInputTypeMeta[measureInput].ru.short}</span>
            </div>
        </div>
    );
};
